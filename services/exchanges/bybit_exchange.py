"""
APEX PROTOCOL™ — Bybit Exchange Service
Независимый модуль сканирования Bybit Futures.
"""
import os
import asyncio
import logging
...
logger = logging.getLogger("apex.exchange.bybit")
...
class BybitExchangeService(BaseExchangeService):
    ...
    async def connect(self):
        ...
        logger.info("[Bybit] connected to Bybit Futures")

    async def get_tickers(self) -> list:
        """Получить все USDT фьючерсные пары с Bybit."""
        ...
        try:
            ...
            logger.info(f"[Bybit] {len(pairs)} pairs loaded")
            return pairs
    ...
    async def get_ohlcv(self, symbol: str, timeframe: str = "15m", limit: int = 100) -> list:
        ...
        try:
            ...
            logger.error(f"[Bybit] get_ohlcv error {symbol}: {e}")
            return []
